...
export const getBranchDashboard = async (req, res) => {
  try {
    ...
    const [branchRes, inventoryRes, productsRes, salesRes, transfersRes, recentSalesRes, recentTransRes] = await Promise.all([
      supabase.from('branches').select('*').eq('id', branchId).single(),
      supabase.from('branch_inventory').select('*').eq('branch_id', branchId),
      supabase.from('products').select('*'),
      supabase.from('sales').select('quantity').eq('branch_id', branchId).gte('created_at', todayStart.toISOString()),
      supabase.from('transfers').select('id').in('status', ['Pending', 'In-transit']).or(`from_branch_id.eq.${branchId},to_branch_id.eq.${branchId}`),
      supabase.from('sales').select('quantity, created_at, product_id').eq('branch_id', branchId).order('created_at', { ascending: false }).limit(5),
      supabase.from('transfers').select('quantity, status, transfer_type, created_at, product_id').or(`from_branch_id.eq.${branchId},to_branch_id.eq.${branchId}`).order('created_at', { ascending: false }).limit(5)
    ]);
    ...
    inventory.forEach(item => {
      ...
      if (!productStockMap[productName]) {
        productStockMap[productName] = { total: 0, unit: product?.unit || '' };
      }
      productStockMap[productName].total += Number(item.stock);
      totalStockItems += Number(item.stock);
    ...
    const aggregatedStock = Object.keys(productStockMap).map(name => ({
      name,
      total: productStockMap[name].total,
      unit: productStockMap[name].unit
    }));

    const todaySalesCount = (salesRes.data || []).reduce((acc, sale) => acc + Number(sale.quantity), 0);
    const activeRedistributionsCount = (transfersRes.data || []).length;

    const recentSales = (recentSalesRes.data || []).map(s => {
      const p = products.find(prod => prod.id === s.product_id);
      return { type: 'sale', desc: `Sold ${s.quantity} ${p?.unit || ''} of ${p?.name || 'Unknown'}`, date: s.created_at };
    });
    const recentTrans = (recentTransRes.data || []).map(t => {
      const p = products.find(prod => prod.id === t.product_id);
      return { type: 'transfer', desc: `Transfer (${t.status}): ${t.quantity} ${p?.unit || ''} of ${p?.name || 'Unknown'}`, date: t.created_at };
    });
    const recentActivities = [...recentSales, ...recentTrans].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 5);
    ...
    res.json({
      ...
      expiredCount,
      todaySalesCount,
      activeRedistributionsCount,
      criticalAlerts: criticalAlerts.slice(0, 5),
      productStocks: aggregatedStock,
      recentActivities
    });
...
export const getBranchSales = async (req, res) => {
  try {
    ...
    const [salesRes, prodRes, invRes] = await Promise.all([
      ...
      supabase.from('products').select('*'),
      supabase.from('branch_inventory').select('id, batch_number')
    ]);

    const salesList = (salesRes.data || []).map(item => {
      const product = (prodRes.data || []).find(p => p.id === item.product_id);
      const inventoryItem = (invRes.data || []).find(inv => inv.id === item.batch_id);
      const dateObj = new Date(item.created_at);
      
      const realBatchNumber = inventoryItem?.batch_number || (item.batch_id ? item.batch_id.substring(0, 12).toUpperCase() : 'N/A');
...
export const createBranchTransfer = async (req, res) => {
  try {
    ...
    let { from_branch_id, to_branch_id, product_id, batch_id, quantity, transfer_type } = req.body;
    
    // Default to send if not provided
    if (!transfer_type) transfer_type = 'send';
    ...
    if (error) {
       ...
       // Fallback without transfer_type if column doesn't exist
...
export const updateBranchTransfer = async (req, res) => {
  try {
    ...
    if (status === 'Completed' && transfer.status !== 'Completed') {
      ...
      // Just updating status to In-transit, stock isn't moved yet.
    }
...
export const getTransferRecommendations = async (req, res) => {
  try {
    ...
    const { product_id, quantity, type } = req.body; // type = 'send' or 'request'
    ...
    if (type === 'send') {
      ...
      const { data: myInv, error: myInvError } = await supabase.from('branch_inventory').select('stock, product_id').eq('product_id', product_id).eq('branch_id', branchId);
      const myStock = (myInv || []).reduce((acc, curr) => acc + Number(curr.stock), 0);
      if (myStock < quantity) {
        return res.status(400).json({ error: `Debug: Stock=${myStock}, Qty=${quantity}, PID=${product_id}, BID=${branchId}. Err=${myInvError?.message || 'none'}. Please share this message!` });
      }
    }
    ...
    if (type === 'request') {
      ...
      recs.sort((a, b) => b.available_stock - a.available_stock); // Highest stock first
    } else {
      // Send: recommend those with lowest stock
      recs.sort((a, b) => a.available_stock - b.available_stock);
    }
    
    console.log(`[getTransferRecommendations] req=${JSON.stringify({product_id, quantity, type, branchId})}, recs=${JSON.stringify(recs)}`);

    if (recs.length === 0) {
      return res.status(400).json({ error: `DebugEmptyRecs: PID=${product_id}, Qty=${quantity}, Type=${type}. Checked ${Object.keys(branchStockMap).length} branches. Share this!` });
    }
